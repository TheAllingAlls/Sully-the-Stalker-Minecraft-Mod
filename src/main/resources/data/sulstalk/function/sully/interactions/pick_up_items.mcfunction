...
execute if score @s sulstalk_can_pick_up_items matches 1 positioned as @s as @e[type=item,tag=!sulstalk_stolen,tag=!sulstalk_dropped,distance=..10] run data merge entity @s {Tags:["sulstalk_stolen"],Item:{components:{"minecraft:custom_data":{"UUID":-1}}},PickupDelay:20s}

##Extra addition to remove custom data when item is near a player (quality of life for players)
execute positioned as @s as @e[tag=sulstalk_stolen] if data entity @s {Item:{components:{"minecraft:item_model":"minecraft:air"}},PickupDelay:0s} if entity @e[type=player,distance=..5] run data remove entity @s Item.components.minecraft:custom_data
...
execute if score @s sulstalk_can_pick_up_items matches 1 positioned as @s rotated as @s as @e[tag=sulstalk_stolen,distance=..2] if score @s sulstalk_item_id = @e[tag=sulstalk_spawned,distance=0..1,limit=1,sort=nearest] sulstalk_spawned_number run tp ^ ^ ^0.01
