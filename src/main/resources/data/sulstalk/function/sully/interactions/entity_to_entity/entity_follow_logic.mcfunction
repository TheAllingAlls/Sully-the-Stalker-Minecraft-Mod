...
execute positioned ~ ~1 ~ as @e[type=player,distance=..1.5] positioned ~ ~-1 ~ if score @s sulstalk_followed_entity_id = @e[limit=1,distance=..0.1,tag=sulstalk_spawned] sulstalk_following_entity_id unless data entity @s {active_effects:[{id:"minecraft:poison"}]} run effect give @s poison 1 10 true
